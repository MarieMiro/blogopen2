...
const STATUS_LABEL = {
  ...
  disputed:    { text: "Спор",            color: "#c0392b", bg: "rgba(192,57,43,0.10)" },
};
...
export default function BloggerDeals() {
  ...
  // Ответ на сделку (принять/отклонить) — для pending
  const onRespond = async (deal, action) => {
    setActing(deal.id);
    try {
      const res = await fetch(`${API_BASE}/api/deals/${deal.id}/respond/`, {
      ...
      setDeals((p) => p.map((d) => d.id === deal.id ? { ...d, status: data.deal.status } : d));
    } catch {
  ...
  const onMarkDone = async (deal) => {
    ...
    try {
      ...
      setDeals((p) => p.map((d) =>
        d.id === deal.id
          ? { ...d, status: data.deal.status, completed_by_blogger: data.deal.completed_by_blogger }
          : d
      ));
    } catch {
      alert("Ошибка соединения с сервером");
  ...
  const goToChat = (convId) => {
    navigate("/dashboard/blogger/messages", { state: { convId } });
  };
    ...
    <div className="deals">
      ...
      {!loading && deals.length === 0 && (
        <div className="deals__empty">
          ...
          <p className="muted small">Когда бренд отправит предложение — оно появится здесь</p>
        </div>
      )}
      ...
      <div className="deals__list">
        {deals.map((deal) => {
          ...
          const canRespond = deal.status === "pending";
          const canMarkDone = ["accepted", "in_progress"].includes(deal.status) && !deal.completed_by_blogger;
          const waitingBrand = ["accepted", "in_progress"].includes(deal.status) && deal.completed_by_blogger;
                ...
                <div className="dealCard__body">
                  ...
                  {waitingBrand && (
                    <div className="dealCard__notice dealCard__notice--info">
                      ⏳ Вы отметили выполнение — ожидаем подтверждения от бренда
                    </div>
                  )}
                  ...
                  <div className="dealCard__actions">
                    ...
                    {canRespond && (
                      <>
                        <button
                          className="dealCard__btn dealCard__btn--accept"
                          onClick={() => onRespond(deal, "accept")}
                          disabled={isActing}
                        >
                          {isActing ? "…" : "✅ Принять сделку"}
                        </button>
                        <button
                          className="dealCard__btn dealCard__btn--decline"
                          onClick={() => onRespond(deal, "decline")}
                          disabled={isActing}
                        >
                          {isActing ? "…" : "❌ Отклонить"}
                        </button>
                    ...
                    {/* Кнопка «Выполнено» — висит пока сделка активна */}
                    {canMarkDone && (
                      <button
