...
export default function BloggerMessages() {
  ...
  const [respondingDeal, setRespondingDeal] = useState(false);
  ...
  // 1) Загрузка диалогов + polling 30 сек
  useEffect(() => {
    let alive = true;
    ...
    return () => { alive = false; clearInterval(dialogsPoll); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // 2) Сообщения + polling 10 сек
  useEffect(() => {
    if (!activeId) return;
    let alive = true;
    ...
    const load = async (isFirst = false) => {
      try {
        ...
        requestAnimationFrame(() => {
          ...
          if (isAtBottomRef.current) listRef.current.scrollTop = listRef.current.scrollHeight;
        });
      } catch {
        if (alive) setError("Ошибка соединения с сервером");
      } finally {
  ...
  const onSend = async (e) => {
    ...
    try {
      ...
      setDialogs((prev) => prev.map((d) =>
        d.id === activeId ? { ...d, last_message: t, last_message_at: new Date().toISOString() } : d
      ));
    } catch {
      setMessages((p) => p.filter((m) => m.id !== tempId));
      setText(t);
  ...
  // 4) Ответ на сделку
  const onRespondDeal = async (action) => {
    if (!pendingDeal || respondingDeal) return;
    setRespondingDeal(true);
    setError("");
    try {
      const res = await fetch(`${API_BASE}/api/deals/${pendingDeal.id}/respond/`, {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { setError(data.error || "Ошибка ответа на сделку"); return; }
      // Обновляем статус локально и перезагружаем сообщения
      setPendingDeal((p) => p ? { ...p, status: data.deal.status } : null);
      // Сбрасываем кэш чтобы polling подхватил системное сообщение
      const msgRes = await fetch(`${API_BASE}/api/chat/${activeId}/messages/`, { credentials: "include" });
      const msgData = await msgRes.json().catch(() => ({}));
      if (msgRes.ok) setMessages(msgData.messages || msgData.results || []);
    } catch {
      setError("Ошибка соединения с сервером");
    } finally {
      setRespondingDeal(false);
    }
  };
    ...
    <div className={`msg ${activeId ? "chat-open" : ""}`}>
      ...
      <section className="msg__left">
        <div className="msg__leftHead">
          <div className="msg__leftTop"><div className="msg__title">Все чаты</div></div>
      ...
      <section className="msg__right">
        ...
        {/* Баннер сделки для блогера */}
        {pendingDeal && pendingDeal.status === "pending" && (
          <div className="msg__dealBannerBlogger">
            <div className="msg__dealBannerInfo">
              <span className="msg__dealBannerTitle">🤝 Предложение о сделке #{pendingDeal.id}</span>
              {pendingDeal.amount && (
                <span className="msg__dealBannerAmount">{pendingDeal.amount}</span>
              )}
              {pendingDeal.deadline && (
                <span className="msg__dealBannerDeadline">📅 {pendingDeal.deadline}</span>
              )}
            </div>
            <div className="msg__dealBannerActions">
              <button
                type="button"
                className="msg__dealAccept"
                onClick={() => onRespondDeal("accept")}
                disabled={respondingDeal}
              >
                {respondingDeal ? "…" : "✅ Принять"}
              </button>
              <button
                type="button"
                className="msg__dealDecline"
                onClick={() => onRespondDeal("decline")}
                disabled={respondingDeal}
              >
                {respondingDeal ? "…" : "❌ Отклонить"}
              </button>
            </div>
          </div>
        )}

        {/* Баннер результата сделки */}
        {pendingDeal && pendingDeal.status !== "pending" && (
          <div className={`msg__dealResult ${pendingDeal.status === "accepted" ? "msg__dealResult--ok" : "msg__dealResult--no"}`}>
            {pendingDeal.status === "accepted"
              ? "✅ Сделка принята — сотрудничество подтверждено!"
              : "❌ Сделка отклонена"}
          </div>
        )}
